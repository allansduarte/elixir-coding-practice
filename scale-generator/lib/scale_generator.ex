defmodule ScaleGenerator do
  ...
  @flat_chromatic_scale ~w(C D# D E# E F G# G A# A B# B)
  ...
  def step(scale, tonic, step) do
    position = Enum.find_index(scale, &(&1 == tonic))

    Enum.at(scale, position + @tonics[step])
  end
  ...
  def chromatic_scale(tonic \\ "C", scale \\ @chromatic_scale) do
  ...
  def flat_chromatic_scale(tonic \\ "C") do
    ...
    |> Enum.map(&Regex.replace(~r/#/, &1, "b"))
    |> (&chromatic_scale(tonic, &1)).()
  end
  ...
  def scale(tonic, pattern) do
    ...
    pattern
    |> String.codepoints()
    |> Enum.reduce([tonic], fn tone, acc ->
      acc ++ [step(scale, List.last(acc), tone)]
    end)
  end
end
