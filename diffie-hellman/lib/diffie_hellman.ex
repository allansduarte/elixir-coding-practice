defmodule DiffieHellman do
  ...
  def generate_public_key(prime_p, prime_g, private_key) do
    powermod(prime_g, private_key, prime_p)
  end
  ...
  def generate_shared_secret(prime_p, public_key_b, private_key_a) do
    powermod(public_key_b, private_key_a, prime_p)
  end

  defp powermod(base, n, m) do
    powermod(base, n, m, 1)
  end

  defp powermod(_, 0, m, acc), do: rem(acc, m)

  defp powermod(base, n, m, acc) do
    case rem(n, 2) do
      0 -> powermod(rem(base * base, m), div(n, 2), m, acc)
      1 -> powermod(base, n - 1, m, rem(acc * base, m))
    end
  end
end
